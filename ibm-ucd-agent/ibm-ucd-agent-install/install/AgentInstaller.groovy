...
class AgentInstaller {
    ...
    def installAgentFullEncryption = null
    ...
    void initInstalledProperties() {
        ...
        if (ant.project.properties.'agent.jms.full_encryption') {
            installAgentFullEncryption = ant.project.properties.'agent.jms.full_encryption'
        }
    ...
    void init() {
        ...
        def overlayZip = srcDir + "/overlay.zip"
    ...
    void installAgent() {
        ...
        if (new File(installAgentDir + "/conf/agent/agent.properties").exists()) {
            ant.move(tofile: installAgentDir + "/conf/agent/installed.properties",
                    file: installAgentDir + "/conf/agent/agent.properties")
        }
        if (new File(installAgentDir + "/conf/agent/installed.properties").exists()) {
            ant.property(file: installAgentDir + "/conf/agent/installed.properties")
            initInstalledProperties()
            ...
            def propsFile = new File(installAgentDir + "/conf/agent/installed.properties")
        ...
        String promptAgentFullEncryption =
            "Enable end-to-end encryption for server/agent JMS communication? This option enhances" +
            " the security of UrbanCode messages sent between the agent and the server, and" +
            " requires an initial HTTPS connection to set up keys."
        String defaultAgentFullEncryption = "y"

        if (doUpgrade) {
            promptAgentFullEncryption = promptAgentFullEncryption + " y,N [Default: N]"
            defaultAgentFullEncryption = "n"
        }
        else {
            promptAgentFullEncryption = promptAgentFullEncryption + " Y,n [Default: Y]"
        }
        installAgentFullEncryption = parseBoolean(prompt(
            installAgentFullEncryption,
        ...
        if (installAgentFullEncryption != null && installAgentFullEncryption) {

            String promptAgentServerUrl = "Enter the full web URL for the central IBM UrbanCode Deploy " +
                "server to validate the connection. Leave empty to skip."
                ...
                while (!serverUrlIsValid) {
                    ...
                    if (installAgentServerUrl == null || installAgentServerUrl.isAllWhitespace()) {
                        ...
                        try {
                            ...
                            def response = executeGetMethod(installAgentServerUrl)
                            serverUrlIsValid = true
                            println("Server response ok.")
                        }
                        catch (Exception e) {
                            println("Error: Server access attempt failed for URL [${installAgentServerUrl}]: " + e + ".\n")
                        }
    ...
    private void copyAgentFiles(installAgentDir, doUpgrade) {
        // In order to run the installer the User had change the encoding of these files. Since we
        // treat all files as UTF-8 we will convert these back to UTF-8 temporarily.
        if (isZos) {
            IO.transcode(systemEncoding, agentEncoding, new File(srcDir + "/opt/apache-ant-1.8.4/bin/ant"))
            IO.transcode(systemEncoding, agentEncoding, new File(srcDir + "/opt/apache-ant-1.8.4/bin/antRun"))
            IO.transcode(systemEncoding, agentEncoding, new File(srcDir + "/opt/groovy-1.8.8/bin/groovy"))
            IO.transcode(systemEncoding, agentEncoding, new File(srcDir + "/opt/groovy-1.8.8/bin/startGroovy"))
        }
        ...
        if (!doUpgrade) {
            ant.propertyfile(file: installAgentDir + "/conf/agent/installed.properties") {
                ...
                entry(key: "agent.jms.full_encryption", value: installAgentFullEncryption)
            ...
            ant.propertyfile(file: installAgentDir + "/conf/agent/installed.properties") {
                ...
                entry(key: "agent.jms.full_encryption", default: installAgentFullEncryption)
    ...
    private void convertFilesToSystemEncoding() {
        def files = []
        File groovyConfDir = new File(installAgentDir + "/opt/groovy-1.8.8/conf")
        File groovyBinDir = new File(installAgentDir + "/opt/groovy-1.8.8/bin")
        File antBinDir = new File(installAgentDir + "/opt/apache-ant-1.8.4/bin")

        groovyConfDir.eachFile { IO.transcode(agentEncoding, systemEncoding, new File(groovyConfDir, it.name)) }
        groovyBinDir.eachFile { IO.transcode(agentEncoding, systemEncoding, new File(groovyBinDir, it.name)) }
        antBinDir.eachFile { IO.transcode(agentEncoding, systemEncoding, new File(antBinDir, it.name)) }
        ...
        // The user had to change the encoding of these files in order to run the installer. We
        // should return them to the system encoding now that we've copied them to the install
        // location.
        IO.transcode(agentEncoding, systemEncoding, new File(srcDir + "/opt/apache-ant-1.8.4/bin/ant"))
        IO.transcode(agentEncoding, systemEncoding, new File(srcDir + "/opt/apache-ant-1.8.4/bin/antRun"))
        IO.transcode(agentEncoding, systemEncoding, new File(srcDir + "/opt/groovy-1.8.8/bin/groovy"))
        IO.transcode(agentEncoding, systemEncoding, new File(srcDir + "/opt/groovy-1.8.8/bin/startGroovy"))

        // chmod opt bin files in source directory after transcode
        ant.chmod(perm: "+x", type: "file") {
            fileset(dir: srcDir) {
                include(name: "opt/*/bin/*")
                exclude(name: "opt/*/bin/*.bat")
