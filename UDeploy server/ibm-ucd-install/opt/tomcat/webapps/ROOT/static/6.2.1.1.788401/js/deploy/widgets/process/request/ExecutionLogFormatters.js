...
function(
    ...
    exports.nameFormatter = function(activity, noLink) {
        ...
        if (activity.specialNameType === "agent") {
            ...
            domConstruct.create("a", {
                ...
                "innerHTML": activity.componentProcess.name.escape()
            }, result);
        ...
        else if (activity.specialNameType === "note") {
            domConstruct.create("div", {
                className: "inline-block execution-log-process-icon process-icon note-step-icon"
            }, result);

            var activityText = activity.text.substring(0, 100);
            if (activityText.length >= 100) {
                activityText = i18n("%s...", activityText);
            }

            util.appendTextSpan(result, activityText.escape()+"&nbsp;");
        }
            ...
            if (activity.command) {
                ...
                var pluginLinkText = i18n("(%s v. %s)", translatedPluginName, pluginVersion);
        ...
        if (status === "CLOSED" && result === "SUCCEEDED") {
            if (activity.notNeeded) {
                ...
                if (activity.notNeeded === "noResources") {
                    ...
                    cellContents = i18n("Resource Offline");
                }
        ...
        if (activity.fault) {
            ...
            faultDialogLink.onclick = function() {
                ...
                if (activity.fault.message) {
                    messageDiv.innerHTML = activity.fault.message;
                }
        ...
        if (activity.task) {
            if (activity.task.status === "OPEN") {
                ...
                on(commentDialogLink, "click", function() {
                    ...
                    userDiv.innerHTML = "by "+task.completedBy+" on "+util.dateFormatShort(task.completedOn);
                ...
                completedSpan.innerHTML = i18n("Completed by %s", activity.task.completedBy);
                ...
                failedSpan.innerHTML = i18n("Failed by %s", activity.task.completedBy);
            ...
            if (activity.state !== "INITIALIZED") {
                ...
                var i = 0;
                headerDiv.insertDiv(i18n("Working Directory"), activity.workingDir, i);
                i++;
                if (activity.username) {
                    headerDiv.insertDiv(i18n("Impersonation User"), activity.username, i);
                    i++;
                }
                ...
                if (activity.group) {
                    headerDiv.insertDiv(i18n("Impersonation Group"), activity.group, i);
                    i++;
                }
        ...
        xhr.get({
            ...
            load: function(activity) {
                if (activity.properties) {
                    var inputPropertiesTable = new Table({
                ...
                if (activity.outputProps) {
                    var outputPropertiesTable = new Table({
    ...
    exports.showErrorDialog = function(activity) {
        ...
        util.appendTextSpan(inputContainer, i18n(activity.exception || activity.error));
