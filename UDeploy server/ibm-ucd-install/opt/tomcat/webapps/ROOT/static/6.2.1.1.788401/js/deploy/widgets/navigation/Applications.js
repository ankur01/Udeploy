...
function(domConstruct,
    ...
    config.data.tabSets.push({
        ...
        getDetailTitle: function() {
            return entities.encode(i18n("Process: %s", appState.applicationProcess.name));
        },
        getDetailFields: function() {
            return [{
                label: i18n("Version"),
                value: i18n("%s of %s", appState.applicationProcess.version, appState.applicationProcess.versionCount)
            },{
                label: "",
                value: util.vc.generateVersionControls(appState.applicationProcess, function(version) {
                    return "applicationProcess/"+appState.applicationProcess.id+"/"+version;
                })
            }];
        },
        getDetailDescription: function() {
            return entities.encode(appState.applicationProcess.description);
        },
        ...
        getDetailFields: function() {
            ...
            if (appState.environment.blueprint) {
                detailProperties.push({
                    ...
                    value: appState.environment.blueprint.name.escape()
                });
            }
