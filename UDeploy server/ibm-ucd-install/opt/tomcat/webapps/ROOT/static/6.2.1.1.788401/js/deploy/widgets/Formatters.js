...
function(array,
    ...
    return {
        ...
        applicationProcessLinkFormatter: function(item) {
            ...
            if (item) {
                ...
                if (item.metadataType === "patternApplicationProcess"){
                    ...
                    link.href = "#applicationProcess/"+item.id+"/"+item.version;
        ...
        genericProcessLinkFormatter: function(item) {
            ...
            if (item) {
                if (!item.security || item.security.read) {
                    result = document.createElement("a");
                    result.innerHTML = item.name.escape();
                    result.href = "#process/"+item.id+"/"+item.version;
                }
                else {
                    result = item.name.escape();
                }
        ...
        componentTemplateProcessLinkFormatter: function(item) {
            ...
            if (item) {
                result.innerHTML = item.name.escape();
                result.href = "#componentProcess/"+item.id+"/"+item.version;
            }
            return result;
        },
        ...
        inventoryGridColumnsFormatter: function(showVersionPopup, callback, showType) {
            var self = this;
            var grid = [];
            if (showVersionPopup) {
                grid = [{
            ...
            grid = grid.concat([{
                ...
                formatter: function(item, result, cell) {
                    ...
                    if (item.additionalVersions && item.additionalVersions.length > 0 && showVersionPopup) {
        ...
        resourceParentFormatter: function(item) {
            ...
            if (showAgent || showAgentPool || showPrototype) {
                domConstruct.create("span", {
                    innerHTML: "&nbsp;&nbsp;("
                }, result);
            }
        ...
        getResourceIcon: function(item) {
            ...
            if (item.agent) {
                ...
                iconClass = "agentPoolIcon";
            }
            else if (item.componentTag) {
                iconClass = "componentIcon";
            }
            else if (item.role) {
                if (item.role.specialType === "COMPONENT") {
                    ...
                    iconClass = "folderGearIcon";
                }
                else if (item.role.specialType === "AGENT_PLACEHOLDER") {
                    iconClass = "agentPlaceholderIcon";
                }
         ...
         */
        createIcon: function(iconClass, altText) {
            var result = domConstruct.create("div", {
                "class": "inlineBlock general-icon " + iconClass,
                "title": altText || ""
            });
        ...
        zosArtifactFormatter: function(item) {
            var result = document.createElement("div", {
                "class": "inlineBlock"
            });
            if (item) {
