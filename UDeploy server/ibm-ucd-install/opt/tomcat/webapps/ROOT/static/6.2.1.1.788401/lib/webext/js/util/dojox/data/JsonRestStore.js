...
*/
dojo.provide("js.util.dojox.data.JsonRestStore");

dojo.require("dojox.data.ServiceStore");
dojo.require("dojox.data.JsonRestStore");
dojo.require("js.util.dojox.rpc.JsonRest");

dojo.declare("js.util.dojox.data.JsonRestStore",
	dojox.data.ServiceStore,
	{
		constructor: function(options){
			...
			if(typeof options.target == 'string'){
				...
				if(!this.service){
					this.service = js.util.dojox.rpc.JsonRest.services[options.target] ||
							dojox.rpc.Rest(options.target, true);
					// create a default Rest service
				}
			}

			js.util.dojox.rpc.JsonRest.registerService(this.service, options.target, this.schema);
			...
			var constructor = js.util.dojox.rpc.JsonRest.getConstructor(this.service);
			var self = this;
		...
		deleteItem: function(item){
			...
			if(this.referenceIntegrity){
				...
				js.util.dojox.rpc.JsonRest._saveNotNeeded = true;
				...
				fixReferences(index);
				js.util.dojox.rpc.JsonRest._saveNotNeeded = false;
				var i = 0;
				while(checked[i]){
			...
			js.util.dojox.rpc.JsonRest.deleteObject(item);
		...
		changing: function(item,_deleting){
			...
			//		revert.
			js.util.dojox.rpc.JsonRest.changing(item,_deleting);
		},
		...
		save: function(kwArgs){
			...
			var actions = js.util.dojox.rpc.JsonRest.commit(kwArgs);
		...
		revert: function(kwArgs){
			...
			js.util.dojox.rpc.JsonRest.revert(kwArgs && kwArgs.global && this.service);
		},

		isDirty: function(item){
			// summary
			//		returns true if the item is marked as dirty.
			return js.util.dojox.rpc.JsonRest.isDirty(item);
		},
		isItem: function(item, anyStore){
			...
			return item && item.__id && (anyStore || this.service == js.util.dojox.rpc.JsonRest.getServiceAndId(item.__id).service);
		},
		_doQuery: function(args){
			var query= typeof args.queryStr == 'string' ? args.queryStr : args.query;
			var deferred = js.util.dojox.rpc.JsonRest.query(this.service,query, args);
			var self = this;
			if(this.loadReferencedSchema){
				deferred.addCallback(function(result){
					...
					if(schemaRef){
						var serviceAndId = js.util.dojox.rpc.JsonRest.getServiceAndId((self.target + schemaRef).replace(/^(.*\/)?(\w+:\/\/)|[^\/\.]+\/\.\.\/|^.*\/(\/)/,"$2$3"));
						var schemaDeferred = js.util.dojox.rpc.JsonRest.byId(serviceAndId.service, serviceAndId.id);
		...
		fetchItemByIdentity: function(args){
			...
			if(id.toString().match(/^(\w*:)?\//)){
				var serviceAndId = js.util.dojox.rpc.JsonRest.getServiceAndId(id);
...
js.util.dojox.data.JsonRestStore.getStore = function(options, Class){
	//	summary:
	...
	if(typeof options.target == 'string'){
		...
		var store = (js.util.dojox.rpc.JsonRest.services[options.target] || {})._store;
		if(store){
			return store;
		}
	}
	return new (Class || js.util.dojox.data.JsonRestStore)(options);
};
dojox.data._getStoreForItem = function(item){
	if(item.__id){
		var serviceAndId = js.util.dojox.rpc.JsonRest.getServiceAndId(item.__id);
		if(serviceAndId && serviceAndId.service._store){
			return serviceAndId.service._store;
		}else{
			var servicePath = item.__id.toString().match(/.*\//)[0];
			return new js.util.dojox.data.JsonRestStore({target:servicePath});
		}
	}
	return null;
};
