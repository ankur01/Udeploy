                ...
                formatter: function(item, value, cell) {
                    ...
                    if (!self.readOnly) {
                        ...
                        self.addEvent = on(add, "click", function(e) {
                            var _this = this;
                            //get the position to place the selector div
                            var positioning = add.getBoundingClientRect();

                            //subtract the padding around the selector div
                            var topPosition = positioning.top - 17;
                            var leftPosition = positioning.left - 200;
                            //set the absolute position programmatically to lay on the correct cell
                            domStyle.set(selectorAttach, {
                                top: topPosition +"px",
                            ...
                            _this.selector.placeAt(selectorAttach);
                            ...
                            var windowClickListener = on(window.doc, "click", function(e) {
                                if (_this.selector
                                        ...
                                        && e.target !== add
                                        && !dom.isDescendant(e.target, selectorAttach)
                                        // The parent of the selector attach and the dropdown (sibling) is the body
                                        // of the page.
                                        && !dom.isDescendant(e.target, selectorAttach.nextSibling)) {
                                    _this.selector.destroy();
                    ...
                    domConstruct.place(selectorAttach, dom.byId("body"));
