...
function(
    ...
    return declare([_Widget, _TemplatedMixin], {
        ...
        getRequirementDescription: function() {
            ...
            if (this.tagRequirement.type === "EQUALS") {
                return i18n("Needs %s components with \'%s\" tag.", String(number),  this.tagRequirement.tag.name);
            }
            if (this.tagRequirement.type === "GREATER_THAN") {
                return i18n("Needs %s or more components with \'%s\" tag.", String(number + 1),  this.tagRequirement.tag.name);
            }
            if (this.tagRequirement.type === "LESS_THAN") {
                return i18n("Needs %s or fewer components with \'%s\" tag.", String(number - 1),  this.tagRequirement.tag.name);
            }
            throw "Unknown tag requirement type: " + this.tagRequirement.type;
        },
